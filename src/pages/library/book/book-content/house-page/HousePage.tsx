...
const HousePage = ({ isSidebarVisible }: SidebarVisibilityProps) => {
  ...
  const { data, isLoading, isError, error } = useQuery(["house", houseId], () =>
        ...
        <div sx={{ ...BookPageStyles, padding: "3rem 3rem 4rem 3rem" }}>
          ...
          >
            ...
            <ul sx={HouseDescriptionListStyles}>
              ...
              <li sx={{ marginBottom: "0.25rem" }}>
                <b>Animal:</b> {data.animal}
              </li>
              <li sx={{ marginBottom: "0.25rem" }}>
                <b>Colors:</b> {data.colors}
              </li>
              <li sx={{ marginBottom: "0.25rem" }}>
                <b>Traits:</b> {data.traits}
              </li>
              <li sx={{ marginBottom: "0.25rem" }}>
                <b>House ghost:</b> {data.ghost}
              </li>
              <li sx={{ marginBottom: "0.25rem" }}>
                <b>Common room:</b> {data.common_room}
              </li>
              <li sx={{ marginBottom: "0.25rem" }}>
              ...
              <li sx={{ marginBottom: "0.25rem" }}>
                <b>Heads:</b>{" "}
                {data.heads.map((head: HouseHeadType, i: number) => (
                  <span key={head.id}>
                    {head.name}
                    {i !== data.heads.length - 1 && ", "}
                  </span>
                ))}
              </li>
            </ul>
