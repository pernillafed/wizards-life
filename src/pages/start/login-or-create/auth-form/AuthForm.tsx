...
import {
  ...
  AuthFormHeadingStyles,
  AuthFormInputFieldStyles,
  AuthFormWrapperStyles,
  ...
  return (
    <div>
      ...
      <form sx={AuthFormWrapperStyles} onSubmit={isLoginForm ? handleLogin : handleCreateAccount}>
        ...
        {error && (
          <div sx={{ color: "salmon", fontSize: "0.9rem", fontFamily: "paragraph" }}>{error}</div>
        )}
        <button type="submit" sx={AuthFormButtonStyles} disabled={loading}>
          {loading ? <BeatLoader margin={1} size={10} color={"#fff3c2c0"} /> : buttonText}
        </button>
      </form>
    </div>
  );
